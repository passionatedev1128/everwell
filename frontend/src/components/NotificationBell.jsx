...
const NotificationBell = () => {
  ...
  return (
    ...
    >
      <button
        onClick={handleOpen}
        className="notification-bell-btn relative p-2 rounded-full hover:bg-primary-dark/30 transition-colors primary-color-text-green"
        title="Notificações"
        style={{
          transform: 'none',
      ...
      >
        <svg className="w-6 h-6 primary-color-text-green" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {isOpen && (
        <div 
          ...
          className="absolute right-0 mt-2 w-80 bg-white shadow-2xl border border-primary/10 py-2 z-50 backdrop-blur-sm"
          style={{
            borderRadius: '24px',
          ...
          <div className="absolute right-4 -mt-2 w-4 h-4 bg-white border-l border-t border-primary/10 transform rotate-45 top-2"></div>
          <div 
          ...
          >
            <div className="flex items-center justify-between">
              <p className="text-sm font-semibold" style={{ color: '#1A1A1A' }}>Notificações</p>
              {notifications.length > 0 && (
                <button
                  ...
                  className="p-1.5 text-xs font-medium hover:bg-primary/10 rounded-md transition-colors flex-shrink-0"
                  style={{ color: '#4A4A4A' }}
          ...
          <div className="max-h-96 overflow-y-auto">
            {loading ? (
              <div className="px-4 py-8 text-center" style={{ color: '#4A4A4A' }}>
                <div className="animate-spin rounded-full h-8 w-8 border-b-2 mx-auto" style={{ borderColor: '#C0DF16' }}></div>
              </div>
            ) : notifications.length === 0 ? (
              <div className="px-4 py-8 text-center" style={{ color: '#4A4A4A' }}>
                ...
                <div key={notification._id}>
                  <div
                    ...
                    className={`px-4 py-3 transition-colors cursor-pointer relative ${
                      !notification.read 
                        ? 'bg-primary/10 hover:bg-primary/15 border-l-4' 
                        : 'hover:bg-primary/5'
                    } ${index < notifications.length - 1 ? 'border-b border-primary/10' : ''}`}
                    style={!notification.read ? { borderLeftColor: '#C0DF16' } : {}}
                  >
                    <div className="space-y-2">
                      <div className="flex items-start gap-2">
                        {!notification.read && (
                          <span className="mt-1.5 w-2 h-2 rounded-full flex-shrink-0" style={{ backgroundColor: '#C0DF16' }}></span>
                        )}
                        <div className="flex items-center gap-2 flex-1">
                          {getTypeIcon(notification.type || 'info')}
                          <p className="text-sm font-semibold" style={{ color: !notification.read ? '#C0DF16' : '#1A1A1A' }}>
                        ...
                        <div className="absolute top-3 right-4 flex items-center gap-2">
                          ...
                          <p className="text-xs" style={{ color: '#4A4A4A', opacity: 0.7 }}>
                      ...
                      <div className="ml-4 space-y-1">
                        <p className="text-xs leading-relaxed pr-16" style={{ color: '#4A4A4A' }}>
          ...
          <div className="px-4 py-3 border-t border-primary/10 space-y-2">
            <button
              ...
              className="w-full text-left px-3 py-2 text-sm font-medium hover:bg-primary/10 rounded-md transition-colors flex items-center gap-2"
              style={{ color: '#C0DF16' }}
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
